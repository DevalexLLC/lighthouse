...
Lighthouse builds with **zero network access**. This document explains what
...
1. **Images** — `ghcr.io/devalexllc/lighthouse-{server,agent,proxy}`,
...
the operator's `LIGHTHOUSE_VERSION` in `.env` selects the compose image tag.
