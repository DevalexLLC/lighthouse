...
**[Security → Report a vulnerability](https://github.com/DevalexLLC/lighthouse/security/advisories/new)**
...
Lighthouse is maintained by a small team. You will get an acknowledgement
...
- the version (`lighthouse-server version`, `lighthouse-agent version`) or
  the commit;
- the deployment shape — containerized control plane, RPM or container
  agent, whether traffic passes the SNI proxy;
- the relevant configuration, with secrets redacted;
- what an attacker gains, and the steps to reproduce it.

## Supported versions

Lighthouse is pre-1.0. Only the latest release receives security fixes;
...
- **Privilege boundaries on the agent host** — the systemd unit, its
  capabilities, and file ownership under the agent's state directory.
...
- **CI's `web-lint` job installs from the npm registry.** It lints SPA
  ...
  every build step is offline. Building the container images or the agent
  RPM does need the network — base images, `apk add libcap`, `dnf install
  rpm-build` — which is why air-gapped sites consume the pre-built
  release bundle rather than rebuilding. `docs/airgap-build.md` lists
