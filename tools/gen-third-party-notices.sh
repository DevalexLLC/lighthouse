...
# Lighthouse binary or image.
#
# Apache-2.0 §4(d) obliges a redistribution to carry the attribution notices
# of the Apache-licensed components inside it, and the MIT/BSD/OFL licenses
# in the tree oblige reproduction of their copyright notices in binary form.
...
#   2. Go modules — vendor/.
#   3. Dashboard SPA — web/dist is embedded into lighthouse-server by
...
cat >"$tmp" <<'HEADER'
Lighthouse — third-party notices
================================

GENERATED FILE — do not edit by hand. Regenerate with `make notices`
(reads vendor/ only; no network).

Lighthouse itself is Copyright 2026 Devalex LLC, licensed under the Apache
License, Version 2.0 — see LICENSE and NOTICE.

The lighthouse-server and lighthouse-agent binaries are statically linked.
They embed the Go standard library and the third-party Go modules listed
below; lighthouse-server additionally embeds the built dashboard, which
...
[ -f "$SPA" ] || {
    ...
    echo "notices: web/dist is embedded in lighthouse-server; run make web to regenerate it" >&2
    exit 1
}
section "Dashboard SPA (bundled into web/dist, embedded in lighthouse-server)"
