...
// expand over ordered site pairs — this agent is always the source — with
// probe_id = UUIDv5(mesh_id, "src|dst|type"), so the same logical direction
// pair yields the same probe_id on every rebuild and config hashes stay
// stable.
func BuildSnapshot(in store.AgentConfigInputs) *pb.ConfigSnapshot {
