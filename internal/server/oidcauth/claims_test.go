...
func TestMapClaims(t *testing.T) {
	admins := []string{"lighthouse-admins", "sre"}
	cases := []struct {
		name     string
		subject  string
		all      map[string]any
		...
		{name: "array claim with admin match", subject: "s1",
			all:      map[string]any{"preferred_username": "alice", "groups": []any{"dev", "lighthouse-admins"}},
		...
		{name: "missing username claim", subject: "s1",
			all:     map[string]any{"groups": []any{"lighthouse-admins"}},
