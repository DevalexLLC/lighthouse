// Package ca implements the Lighthouse built-in certificate authority.
//
// The CA signs two kinds of certificates:
//   - agent client certificates (identity in a URI SAN, lighthouse://agent/<uuid>)
...
func AgentURISAN(agentID uuid.UUID) *url.URL {
	return &url.URL{Scheme: "lighthouse", Host: "agent", Path: "/" + agentID.String()}
}
...
func AgentIDFromCert(cert *x509.Certificate) (uuid.UUID, error) {
	for _, u := range cert.URIs {
		if u.Scheme == "lighthouse" && u.Host == "agent" && len(u.Path) > 1 {
	...
	return uuid.Nil, errors.New("certificate has no lighthouse://agent/<uuid> URI SAN")
}
...
func Init(dir string, ifMissing bool) error {
	...
	tmpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "Lighthouse CA", Organization: []string{"Lighthouse"}},
