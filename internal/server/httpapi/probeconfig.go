...
func (a *api) handleTargetPost(w http.ResponseWriter, r *http.Request) {
	...
	if in.Address == "" && in.URL == "" {
		problems = append(problems, "address or url is required")
	}
...
func validateProbeRequest(in probeRequest, probeType pb.ProbeType, mesh bool) []string {
	...
	problems := probeadmin.ValidateSettings(interval, timeout, int(in.TrainCount), spacing, probeAPIFields)
...
func (a *api) handleProbePost(w http.ResponseWriter, r *http.Request) {
	...
	var id uuid.UUID
	if meshMode {
		id, err = a.db.AddMeshProbe(r.Context(), in.Mesh, in.settings(probeType), updatedBy)
	} else {
		id, err = a.db.AddDirectProbe(r.Context(), in.Site, in.Target, in.settings(probeType), updatedBy)
	}
	if err != nil {
