...
SITE="${LIGHTHOUSE_SITE:?LIGHTHOUSE_SITE must be set}"
CONFIG=/etc/lighthouse/agent.yaml
PKI=/var/lib/lighthouse-agent/pki
...
if [ ! -s "$PKI/agent.crt" ]; then
    ...
    lighthouse-agent enroll --config "$CONFIG" \
        --token "$(cat "$TOKEN_FILE")" --ca-cert /bootstrap/ca.crt \
        --probe-address "agent-${SITE}"
fi

exec lighthouse-agent run --config "$CONFIG"
